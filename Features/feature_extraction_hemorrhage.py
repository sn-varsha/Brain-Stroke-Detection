...
IMG_DIR = r"D:\Stroke_Detection-and-Segmentation-by-Using-CNN-ML\hemorrhage\PNG"  # Brain images
MASK_DIR = r"D:\Stroke_Detection-and-Segmentation-by-Using-CNN-ML\hemorrhage\MASK"   # The folder containing the masks
features_list = []

for fname in sorted(os.listdir(IMG_DIR)):
    img_path = os.path.join(IMG_DIR, fname)
    mask_path = os.path.join(MASK_DIR, fname)  # Assuming mask name is the same as the image name
    ...
    # Get the non-zero pixels inside the masked region
    ...
    # Downscale and normalize for GLCM (reduce from 256 levels to fewer levels)
